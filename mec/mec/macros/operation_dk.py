...
# Gate-valves
## Target Chamber venting valves
GVventingCDA = EpicsSignal('MEC:TC1:VGC:11:OPN_SW')
GVventingCDA_chk = EpicsSignal('MEC:TC1:VGC:11:OPN_OK')
GVventingAtm = EpicsSignal('MEC:TC1:VGC:12:OPN_SW')
GVventingAtm_chk = EpicsSignal('MEC:TC1:VGC:12:OPN_OK')
## Primary vacuum line valves
GVprimaryTC = EpicsSignal('MEC:TC1:VGC:04:OPN_SW')
GVprimaryTC_chk = EpicsSignal('MEC:TC1:VGC:04:OPN_OK')
GVventingPrimary = EpicsSignal('MEC:MEZ:VGC:18:OPN_SW')
GVventingPrimary_chk = EpicsSignal('MEC:MEZ:VGC:18:OPN_OK')
## Ebara pumps valves
GVebara1 = EpicsSignal('MEC:MEZ:VGC:17:OPN_SW')
GVebara1_chk = EpicsSignal('MEC:MEZ:VGC:17:OPN_OK')
GVebara2 = EpicsSignal('MEC:MEZ:VIC:19:OPN_SW')
GVebara2_chk = EpicsSignal('MEC:MEZ:VIC:19:OPN_OK')
## Turbo 2 line valves
GVturbo2 = EpicsSignal('MEC:TC1:VGC:02:OPN_SW')
GVturbo2_chk = EpicsSignal('MEC:TC1:VGC:02:OPN_OK')
GVscroll2 = EpicsSignal('MEC:SEW:VGC:15:OPN_SW')
GVscroll2_chk = EpicsSignal('MEC:SEW:VGC:15:OPN_OK')

# Pumps
## Ebara pumps
ebara1 = EpicsSignal('MEC:MEZ:PMF:01:MPSTART_SW')
ebara2 = EpicsSignal('MEC:MEZ:PMF:02:MP_START_SW')
## Scroll 2
scroll2 = EpicsSignal('MEC:PR60:PWR:1:Outlet:2:SetControlAction')
## Turbo 2
turbo2 = EpicsSignal('MEC:TC1:PTM:02:START_SW')

# Pressure gauges
## Primary pressure gauge
PGprimary = EpicsSignal('MEC:MEZ:GPI:02:PMON')
## Target chamber Pirani gauge
PGchamber = EpicsSignal('MEC:TC1:GPI:01:PMON')
## Scroll 2 gauge
PGscroll2 = EpicsSignal('MEC:SEW:GPI:04:PMON')

def pumpdown():
    print('Initiating pump-down procedure')
    if (GVventingCDA_chk.get() == 0 or GVventingAtm_chk.get() == 0):
        print('- target chamber is not properly vented')
        print('- - interrupting pumping cycle')
        print('- - - vent the chamber before retrying')
        return
    if (GVprimaryTC_chk.get() == 0 and GVventingPrimary_chk.get() == 1):
        print('- primary vacuum line is not properly vented')
        print('- - initiating primary vacuum line venting, wait max. 5 min')
        GVebara1.put(0)
        GVebara2.put(0)
        GVventingPrimary.put(1)
        timecount=0
        while (PGprimary.get() < 650.):
            time.sleep(1)
            timecount+=1
            if (timecount > 5*60):
                print('- - - something is wrong with primary vacuum line venting')
                print('- - - - interrupting pumping cycle')
                print('- - - - - vent the primary line before retrying')
                return
    print('.opening primary vacuum line to target chamber gate valve')
    GVprimaryTC.put(1)
    time.sleep(5)
    print('..starting Ebara 1 pre-pump')
    ebara1.put(1)
    time.sleep(10)
    timecount=0
    print('- waiting until Ebara 1 gate-valve allowed to open, wait max. 1 min')
    while (GVebara1_chk.get() != 1):
        time.sleep(1)
        timecount+=1
        if (timecount > 30):
            print('- - something is wrong with Ebara 1 gate-valve')
            print('- - - interrupting pumping cycle')
            print('- - - - fix before retrying')
            return
    GVebara1.put(1)
    print('..starting Ebara 2 pre-pump')
    ebara2.put(1)
    time.sleep(10)
    timecount=0
    print('- waiting until Ebara 2 gate-valve allowed to open, wait max. 1 min')
    while (GVebara2_chk.get() != 1):
        time.sleep(1)
        timecount+=1
        if (timecount > 30):
            print('- - something is wrong with Ebara 2 gate-valve')
            print('- - - interrupting pumping cycle')
            print('- - - - fix before retrying')
            return
    GVebara2.put(1)
    print('...primary vacuum pumping is now ongoing, monitor chamber pressure')
    if (turbo2.get() != 1):
        print('- turbo 2 is not running')
        print('- - initiating turbo 2 startup procedure')
        print('- - .starting scroll 2')
        GVscroll2.put(0)
        scroll2.put(1)
        time.sleep(90)
        timecount=0
        while (scroll2.get() != 1):
            ...
            timecount+=1
            if (timecount > 30):
                print('- - - something is wrong with scroll 2 status')
                print('- - - - interrupting pumping cycle')
                print('- - - - - debug manually')
                return
        timecount = 0
        while (PGscroll2.get() > 1.e-2):
            time.sleep(1)
            timecount+=1
            if (timecount > 2*60):
                print('- - - something is wrong with scroll 2 pressure')
                print('- - - - interrupting pumping cycle')
                print('- - - - - debug manually')
                return
        if (GVscroll2_chk.get() != 1):
            print('- - - something is wrong with scroll 2 gate-valve')
            print('- - - - interrupting pumping cycle')
            print('- - - - - debug manually')
            return
        print('- - ..opening scroll 2 gate-valve')
        GVscroll2.put(1)
        time.sleep(90)
        print('- - ...starting turbo 2')
        turbo2.put(1)
        timecount=0
        while (turbo2.get() != 1):
            time.sleep(1)
            timecount+=1
            if (timecount > 5*60):
                print('- - - something is wrong with turbo 2')
                print('- - - - interrupting pumping cycle')
                print('- - - - - debug manually')
                return
    print('- wait max. 40 min')
    timecount=0
    while (GVturbo2_chk.get() != 1):
        time.sleep(1)
        timecount+=1
        if (timecount > 40*60):
            if (turbo2.get() == 0):
                print('- - turbo 2 turned off unexpectedly')
                print('- - - interrupting pumping cycle')
                print('- - - - debug manually')
                return
            elif (PGchamber.get() > 5.e-2):
                print('- - cannot reach base pressure in a timely manner')
                print('- - - interrupting pumping cycle')
                print('- - - - debug manually')
                return
            else:
                print('- - something is wrong, cannot open turbo gate-valve')
                print('- - - interrupting pumping cycle')
                print('- - - - debug manually')
    print('....opening turbo gate-valve')
    GVturbo2.put(1)
    time.sleep(60)
    if (GVturbo2.get() == 1 and turbo2.get() == 1):
        print('.....pumping cycle completed')
    else:
        print('- something went wrong when opening turbo gate-valve')
        print('debug manually')
