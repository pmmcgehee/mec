...
meclas_LOSCtemp20220720 intended for situation where LeCroyA is dead and LeCroyB is used temporarily for a hot spare
...
class LPL:
    ...
    def _weichall():
        ...
        """
        try:
            LAchall=LOSC('b').rchall();#changeda->b
            LBchall=LOSC('b').rchall();L2chall=LOSC('2').rchall();
    ...
    def psefc10Hz(cls,pwt='curr',numIterQ=50,AQQ=0.03,displayPlot=True,reloopPrompt=True,YFEbkgrdY=-.004,PtNumFront=3,PtNumBack=2,CorrFactorFront=.97,CorrFactorBack=1.0,avgfwhm=9,avgrange=1):
        ...
        try:
            SLA=LOSC('b');#changeda->b
            efc.ssleep();SLA._Open();efc.ssleep();SH=HAWG();efc.ssleep();SH._Open();efc.ssleep();#replaced w/LeCroyA
    ...
    def SHG_opt(cls,armsQ='ABEFGHIJ'):#check for trace height;#All shutters must start in the open state... 
        ...
        try:
            tempchk1=LOSC('b').rch(1);time.sleep(.15);tempchk2=LOSC('b').rch(1);#changeda->b
        ...
        try:
            SLA=LOSC('b');SLA._Open();#changeda->b
            for ii in range(4):
