...
s500mm=EpicsSignal('MEC:PPL:MMN:14')
...
talbotStage = Motor('MEC:PPL:MMN:24', name='talbotStage')

# method to take postshot reference Talbot IN/OUT
def post_ref_talbot(nEvent=20, SiT_1=1.0, SiT_2=1.0, talbotIN=-3.25, talbotOUT=-49.0):
    pulse_picker(rate=1)
    talbotStage.umv(talbotIN)
    ref_only(xray_trans=SiT_1, xray_num=nEvent, visar=True, save=True, calibrant='white-field_IN')
    talbotStage.umv(talbotOUT)
    ref_only(xray_trans=SiT_2, xray_num=nEvent, visar=True, save=True, calibrant='white-field_OUT')

# method to take preshot reference Talbot OUT/IN
def pre_ref_talbot(nEvent=20, SiT_1=0.1, SiT_2=1.0, talbotIN=-3.25, talbotOUT=-49.0):
    pulse_picker(rate=1)
    talbotStage.umv(talbotOUT)
    ref_only(xray_trans=SiT_1, xray_num=nEvent, visar=True, save=True, calibrant='cold-sample_OUT')
    talbotStage.umv(talbotIN)
    ref_only(xray_trans=SiT_2, xray_num=nEvent, visar=True, save=True, calibrant='cold-sample_IN')
...
def spl_shot(nshot=1, gaia_offset=0., delay=0.0e-9, xray_threshold=0.1, xray_trans=1, msg='', tags_words=['optical', 'sample'], auto_trig=True, save_data=True, freeze_daq=False):
    '''
    Description: script to shoot the optical laser and time it with the xrays. It automatically push to the elog the laser energy, the timing and the xray SiT transmission.
    IN:
...
#        logger.critical('XPP CCM is IN')
    ...
    #print("JJ slit : ({:.3f}, {:.3f})".format(mec_jj_1.xwidth.position, mec_jj_1.ywidth.position))
...
powermeter_stage=Motor('MEC:TC1:MMS:26', name='powermeter_stage')
vacuum_iris=Motor('MEC:USR:MMS:21', name='vacuum_iris')
imaging_syst=Motor('MEC:USR:MMS:22', name='imaging_syst')

def pm(position = 'IN', wait_mvt = True):
    if (position == 'IN'):
        powermeter_stage.umv(-1)
        logger.warning('Power meter is IN')
    if (position == 'OUT'):
        powermeter_stage.umv(-220)
        logger.success('Power meter is OUT')
    if (position == 'FILTER'):
        powermeter_stage.umv(-101)
        logger.success('Filter at PM is IN')

def iris_move(position = 'CLOSE'):
    if (position == 'CLOSE'):
        vacuum_iris.umv(-0.82)
...
def foc_img_move(position ='OUT'):
    if (position == 'OUT'):
        imaging_syst.umv(146.8983)
        logger.success('Imaging system lens is OUT')
    if (position == 'IN'):
        imaging_syst.umv(26.8983)
