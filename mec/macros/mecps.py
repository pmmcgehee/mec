#((once finished, rename as mecps in folder and then transfer to final location))#/macros/
# -*- coding: utf-8 -*-
...
#
def TraceFormatting(PDTrace, PDFETMap, MaxPDValue, AvgRange=25, FWHM=4): #turns PD trace into windowed, averaged list of 140 pts#TraceFormatting2 has AvgRange=1,FWHM=1
...
#used in wIter2 and psefc10Hz, so we should keep this one
...
def eplcomp(listq,goalq,Map,tMax,xlb='none',ylb='none'):
    formtra=[]
    formtra.append(TraceFormatting(listq,Map,tMax, AvgRange=1, FWHM=1))
...
def epllt(listq,Map,xlb='none',ylb='none'):
    ...
    for ii in range(len(listq)):
        formtra.append(TraceFormatting(listq[ii],Map[ii],1, AvgRange=1, FWHM=1))
    epll(formtra,xlb=xlb,ylb=ylb)
    return
#    
def epllcomp(listq,goalq,Map,tMax,xlb='none',ylb='none'):
    formtra=[]
    formtra.append(TraceFormatting(listq[-1],Map,tMax, AvgRange=1, FWHM=1))
    formtra.append(goalq)
    epll(formtra,xlb=xlb,ylb=ylb)
    return
#    
...
def rgen():
    ...
    return
#    
def FixEdges(WavF,DurationListQ,StartStopListQ,CorrFactorFront=1.,CorrFactorBack=1.):
    FirstPix=0#was 50
    DurListQ=np.cumsum([0]+DurationListQ)
    fWavF=WavF[:]
    fWavF[FirstPix+1]=fWavF[FirstPix+2]*CorrFactorFront
    fWavF[FirstPix]=fWavF[FirstPix+1]*CorrFactorFront
    DisconCount=0
    ContCount=0
    ...
    fWavF[FirstPix+int(4*DurListQ[-1])-2-ContCount]=fWavF[FirstPix+int(4*DurListQ[-1])-3-ContCount]*CorrFactorBack
    fWavF[FirstPix+int(4*DurListQ[-1])-1-ContCount]=fWavF[FirstPix+int(4*DurListQ[-1])-2-ContCount]*CorrFactorBack
...
def FixEdges2(WavF,DurationListQ,StartStopListQ,CorrFactorFront=1.05,CorrFactorBack=1.05):
    FirstPix=0#was 50
    DurListQ=np.cumsum([0]+DurationListQ)
    fWavF=WavF[:]
    fWavF[FirstPix+1]=fWavF[FirstPix+2]*CorrFactorFront
...
#keep it just because it's inside the scan and shift utility
...
def EG():
    ...
    eij=pveij.get()
    EAB,EEF,EGH,EIJ=round(1.045*.8113*1.159*1.05*1.17*eab/.00760/1.006/1.0412/1.0799/1.0478,4),round(1.067*.7787*1.089*1.13*.860*eef/.00686/1.006/.9634/0.8410/0.9517,4),round(1.206*1.239*.8701*.914*.897*egh/.00655/1.015/.9692/0.883/0.9650,4),round(0.9841*1.283*.7522*1.07*1.25*eij/.00608/1.015/1.1232/1.075/1.0863,4)#last update 20220128
...
def findfid(TraceInQ): 
    ...
    if (TQP[-1]-TQP[-2]) > 1000:
        ...
        return TQP[-1]
#not used but left here for historical reasons
...
def wIter2(sQ,wQ,DurationListQ,StartStopListQ,zzJQ,mapnowQ,stepQQ):#same as above but with TraceFormatting2, which has less averagingTraceFormatting2
    DurListQ=np.cumsum([0]+DurationListQ)
    ...
    wnew2=FixEdges(UpdatingShapingAlgorithm(PGQ,TraceFormatting(sQ,mapnowQ,PMQ,AvgRange=1,FWHM=1), wQ,stepQQ),DurationListQ,StartStopListQ)
    #epll([wQ[w1:w2],wnew2[w1:w2],np.array(TraceFormatting(sQ,mapnowQ,PMQ))[w1:w2]*.6,np.array(PGQ)[w1:w2]*.6])
    epll([0.*np.array(wnew2[w1:w2]),np.array(wnew2[w1:w2])-np.array(wQ[w1:w2]),np.array(TraceFormatting(sQ,mapnowQ,PMQ,AvgRange=1,FWHM=1))[w1:w2]*.6,np.array(PGQ)[w1:w2]*.6])
    return wnew2
...
def psefc10Hz(pwt=0,numIterQ=50,AQQ=0.03,displayPlot=True,reloopPrompt=True,YFEbkgrdY=0):#was B; replaced when LeCroyA fixed
    ...
    SSs=pickle.load(open(psfpQ+'SSs.p','rb'))
    Psns=pickle.load(open(psfpQ+'Psns.p','rb'))
    pwtF=np.array(TraceFormatting(pwt,[25,500],1,AvgRange=1,FWHM=1))

    try:
        SLA=LXOpen('A');time.sleep(.15);S=HOpen();time.sleep(.15);#replaced w/LeCroyA
        ops00=rch(1,SLA)-YFEbkgrdY;time.sleep(0.1);

        meanerr=[]
        meanerr.append(np.sum(np.abs(pwtF[:26]-TraceFormatting(ops00,[5,100],1,AvgRange=1,FWHM=1)[:26])/pwtF[:26])/len(pwtF[:26]))
        ops00F=TraceFormatting(ops00,[5,100],1,AvgRange=1,FWHM=1)
        print('Start loop')
        if displayPlot:
            plt.ion()
        ...
        while not LoopIsDone:
            for ii in range(numIterQ):
                ...
                ops0=rch(1,SLA)-YFEbkgrdY;time.sleep(0.025);####added.215 when 200mV/div instead of 100mV/div
                if all(ops0 == ops00):
                    ...
                    ops0F=TraceFormatting(ops0,[5,100],1,AvgRange=1,FWHM=1)
                    #epll([pwtF,ops0F])
...
def psrefrwvfm(RecipeStrQ,numStepsQ=50,stepSizeQ=0.25,displayPlot=True,reloopPrompt=True):
    pshostcheck()
    pvMBCpower=EpicsSignal('MEC:64B:PWR:2:Outlet:8:SetControlAction')#WAS 'MEC:S60:PWR:01:Outlet:7:SetControlAction'#read AND write:1=ON,2=OFF
    pvMBCmode=EpicsSignal('MEC:LPL:MBC:01:RunningMode_RBV',write_pv='MEC:LPL:MBC:01:RunningMode')#AUTO=0,MAN=1
    pvMBCsetpt=EpicsSignal('MEC:LPL:MBC:01:AutoCalibration.VAL',write_pv='MEC:LPL:MBC:01:AutoCalibration') #QUAD=0,MIN=1,MAX=2
    ...
    try:#used to make sure shutters re-open even in case of error or KeyboardInterrupt
        ...
        psefc10Hz(pwt=yfegoal,numIterQ=numStepsQ,AQQ=stepSizeQ,displayPlot=displayPlot,reloopPrompt=reloopPrompt,YFEbkgrdY=psYFEbkgrdY)
...
def plzchkcmp(cmpnam):
    try:
        ...
        msgout=['{:<15}'.format(cmpnam[1])+'{:<28}'.format(cmpnam[0])+cstr('{:<6}'.format('fail!'),'blink,r')]
    return msgout
...
def pldemo():
    ...
    while cont:
        ...
        fig.canvas.draw_idle()
        plt.pause(0.025)
        print('Use arrow keys to tune! Use wasd to change step! Use Esc key to quit!')
        qq=getch()
        if qq==keybd()['key_Dn']:
            ypos-=ydel
...
def wPV(yourPV, yourVal):
    ...
    return 
#    
def quickCAM(CAMreq,ImageNo=2,xlb='none',ylb='none',LIVE=False,MAXLOOPS=25):#like MEC:GIGE:31
    PVhead=CAMname(CAMreq)
    try:#if True:#try:
        tres1=rPV(PVhead+':IMAGE'+str(ImageNo)+':ArraySize1_RBV')
        tres2=rPV(PVhead+':IMAGE'+str(ImageNo)+':ArraySize2_RBV')
        twf=rPV(PVhead+':IMAGE'+str(ImageNo)+':ArrayData')
    ...
    if LIVE:
        while loopcount<MAXLOOPS:
            try:
                CAMconfig(PVhead,LIVE=True)
                twf=rPV(PVhead+':IMAGE'+str(ImageNo)+':ArrayData')
                if PVhead == 'MEC:GIGE:29':
                    tres1=rPV(PVhead+':IMAGE'+str(ImageNo)+':ArraySize1_RBV')
                    tres2=rPV(PVhead+':IMAGE'+str(ImageNo)+':ArraySize0_RBV')
                    tres3=rPV(PVhead+':IMAGE'+str(ImageNo)+':ArraySize2_RBV')
...
def threaddemo():
    ...
    thread = threading.Thread(target=Thread_Function, args=(running,'t1'))
    thread2 = threading.Thread(target=Thread_Function3, args=(running, interfunc1, ('sam','dave'), 3, 200))
    ...
    print("EXIT __main__")
#    
def CAMview(*CAMargs,ImageNo=2,LIVE=False,MAXLOOPS=10):
    if CAMargs == ('all',):
...
def setdynxhair(PVhead):
    pass
#   
def CAMconfig(CAMreq, RefXhairXY=[-1,-1], InstructionStr='', MisalignmentTolerance=-1, ImageNo=2, RefXhairXYsize=[-1,-1],LIVE=False):
    ...
    PVhead=CAMname(CAMreq)
    ArraySize=[rPV('{}:IMAGE{}:ArraySize0_RBV'.format(PVhead,ImageNo)),rPV('{}:IMAGE{}:ArraySize1_RBV'.format(PVhead,ImageNo))]
    NameList=['Ref X-hair','DynCentroid','CAMname','Instructions','TimeStamp','Stats','Counts',
              'AlignmentOK'+str(str(MisalignmentTolerance).zfill(3) if MisalignmentTolerance > 0 else rPV('{}:IMAGE{}:Cross4:Name'.format(PVhead,ImageNo))[-3:])]
    ...
    [OverlayName+':SizeXDummy', 9999, OverlayName+':PositionXDummy', 10, '', 0, 
     ...
     '{:<10.9} {:<11.10} {:<11.11}'.format(*CAMname(PVhead,returnAll=True))],
    #Instructions
    [OverlayName+':SizeXDummy', 9999, OverlayName+':PositionXDummy', 10, '', 0, 
     OverlayName+':SizeYDummy', 9999, OverlayName+':PositionYDummy', -20+ArraySize[1], '', 0, 
     InstructionStr if len(InstructionStr)>1 else rPV('{}:IMAGE{}:Box4:DisplayText'.format(PVhead,ImageNo))],
    #TimeStamp
    ...
    [OverlayName+':SizeXDummy', 9999, OverlayName+':XPosDummy', -300+ArraySize[0], '', 0, 
     ...
     'Stats: X:{:<3.3} Y:{:<3.3} sX:{:<3.3} sY:{:<3.3}'.format(str(rPV(PVhead+':Stats2:CentroidX_RBV')), str(rPV(PVhead+':Stats2:CentroidY_RBV')), str(rPV(PVhead+':Stats2:SigmaX_RBV')), str(rPV(PVhead+':Stats2:SigmaY_RBV')))],
    #Counts
    [OverlayName+':SizeXDummy', 9999, OverlayName+':XPosDummy', -282+ArraySize[0], '', 0, 
     ...
     'Max:{:<5} Counts:{:<12}'.format(rPV(PVhead+':Stats2:MaxValue_RBV'), int(rPV(PVhead+':Stats2:Net_RBV')))],
    #Alignment OK
    [OverlayName+':SizeXDummy', 9999, '', 0, OverlayName+':XPosDummy', rPV('{}:IMAGE{}:Box1:CenterX_RBV'.format(PVhead,ImageNo)), 
     OverlayName+':SizeYDummy', 9999, '', 0, OverlayName+':YPosDummy', rPV('{}:IMAGE{}:Box1:CenterY_RBV'.format(PVhead,ImageNo))+rPV('{}:IMAGE{}:Box1:SizeY_RBV'.format(PVhead,ImageNo))+40, 
     'AlignChk']
    ]
    ...
    for ii in range(8):#eight overlays possible
        if (LIVE==True and ii<5):
            ...
            try:
                #turn on stats...
                wPV(PVhead+':Stats{}:EnableCallbacks'.format(ImageNo), 1)
                wPV(PVhead+':Stats{}:ComputeCentroid'.format(ImageNo), 1)
                wPV(PVhead+':Stats{}:ComputeStatistics'.format(ImageNo), 1)
                #set up everything...
                OverlayNameField='{}{}'.format('Box' if ii<4 else 'Cross',(ii%4)+1)
                OverlayName='{}:IMAGE{}:{}'.format(PVhead,ImageNo,OverlayNameField)
                wPV(OverlayName+':Name', NameList[ii])#simple description field
                wPV(OverlayName+':Use', 1)#0=off,1=on
                wPV(OverlayName+':Shape', ShapeList[ii])#0=Cross,1=Rect,2=Text,3=Ellipse;;;DOCUMENTATION WRONG!! 2=Ellipse, 3=Text!!!!
                wPV(OverlayName+':DrawMode', 1)#0=set,1=XOR
                wPV(OverlayName+':Green', 2000 if (ii < 5 or LIVE == True) else 0)#set mono color; don't let last ones appear yet
                if ShapeList[ii] == 0:
                    Width=3#1
                elif ShapeList[ii] == 1:
                    Width=3
                ...
                wPV(OverlayName+':WidthX', Width)#width param for shape lines
                wPV(OverlayName+':WidthY', Width)#width param for shape lines
                ...
                wPV(OverlayName+':SizeXLink.DOL', ArgsList[ii][0].replace('XXXX',OverlayNameField))
                if ArgsList[ii][1] > 0:
                    if ii==7:#special case for Alignment OK
                        ArgsList[ii][12]='Alignment {}'.format('NOT OK' if np.sqrt((rPV(PVhead+':Stats2:CentroidX_RBV')-rPV('{}:IMAGE{}:Box1:CenterX_RBV'.format(PVhead,ImageNo)))**2+(rPV(PVhead+':Stats2:CentroidY_RBV')-rPV('{}:IMAGE{}:Box1:CenterY_RBV'.format(PVhead,ImageNo)))**2) > int(rPV('{}:IMAGE{}:Cross4:Name'.format(PVhead,ImageNo))[-3:]) else 'OK')
                    wPV(OverlayName+':SizeX', ArgsList[ii][1] if ArgsList[ii][1] < 9999 else 9*len(ArgsList[ii][12]))
                wPV(OverlayName+':PositionXLink.DOL', ArgsList[ii][2].replace('XXXX',OverlayNameField))
                if ArgsList[ii][3] > 0:
                    wPV(OverlayName+':PositionX', ArgsList[ii][3])
                    
                wPV(OverlayName+':CenterXLink.DOL', ArgsList[ii][4].replace('XXXX',OverlayNameField))
                if ArgsList[ii][5] > 0:
                    wPV(OverlayName+':CenterX', ArgsList[ii][5])
                
                wPV(OverlayName+':SizeYLink.DOL', ArgsList[ii][6].replace('XXXX',OverlayNameField))
                if ArgsList[ii][7] > 0:
                    wPV(OverlayName+':SizeY', ArgsList[ii][7] if 9999 > ArgsList[ii][7] >= 0 else 20)
                wPV(OverlayName+':PositionYLink.DOL', ArgsList[ii][8].replace('XXXX',OverlayNameField))
                if ArgsList[ii][9] > 0:
                    wPV(OverlayName+':PositionY', ArgsList[ii][9])
                wPV(OverlayName+':CenterYLink.DOL', ArgsList[ii][10].replace('XXXX',OverlayNameField))
                if ArgsList[ii][11] > 0:
                    wPV(OverlayName+':CenterY', ArgsList[ii][11])
                
                if ShapeList[ii] == 3:#only needed for text boxes; NOT 2!!!
                    #use :DisplayText??
                    if ArgsList[ii][12][0] != '%':
                        if (len(ArgsList[ii][12]) > 1):
                            wPV(OverlayName+':DisplayText', ArgsList[ii][12])
                    else:
                        wPV(OverlayName+':TimeStampFormat', ArgsList[ii][12])
                    wPV(OverlayName+':Font',3)
            except:
                print('Error setting up {}!'.format(NameList[ii]))
#
def CAMconfigCurrent():
    CAMconfig(CAMreq='Legend',RefXhairXY=[359,251],MisalignmentTolerance=40,ImageNo=2, RefXhairXYsize=[100,100],LIVE=False,InstructionStr='DO NOT tune to xhair!')
    CAMconfig(CAMreq='StrInA',RefXhairXY=[335,257],MisalignmentTolerance=25,ImageNo=2, RefXhairXYsize=[40,40],LIVE=False,InstructionStr='Use SM0 (NOT SM1!) to put centroid on xhair!')
    CAMconfig(CAMreq='StrInB',RefXhairXY=[334,243],MisalignmentTolerance=40,ImageNo=2, RefXhairXYsize=[40,40],LIVE=False,InstructionStr='Use SM2 to put centroid on xhair!')
    CAMconfig(CAMreq='MPA1In',RefXhairXY=[152,147],MisalignmentTolerance=15,ImageNo=2, RefXhairXYsize=[25,25],LIVE=False,InstructionStr='Use SM3 to put centroid on xhair!')
    CAMconfig(CAMreq='MPA1Out',RefXhairXY=[411,249],MisalignmentTolerance=15,ImageNo=2, RefXhairXYsize=[40,40],LIVE=False,InstructionStr='Use SM4 to maximize spot!')
    CAMconfig(CAMreq='MPA2In',RefXhairXY=[366,276],MisalignmentTolerance=15,ImageNo=2, RefXhairXYsize=[50,50],LIVE=False,InstructionStr='Use SM5(V) to put centroid on xhair!')
    CAMconfig(CAMreq='MPA2Out',RefXhairXY=[385,259],MisalignmentTolerance=40,ImageNo=2, RefXhairXYsize=[190,190],LIVE=False,InstructionStr='Use SM7 (and SM6) to put centroid on xhair!')
    CAMconfig(CAMreq='CompIn',RefXhairXY=[276/2,252/2],MisalignmentTolerance=50,ImageNo=2, RefXhairXYsize=[40,40],LIVE=False,InstructionStr='Use SM8 to put centroid on xhair!')
    CAMconfig(CAMreq='CompOutFF',RefXhairXY=[292,300],MisalignmentTolerance=25,ImageNo=2, RefXhairXYsize=[40,40],LIVE=False,InstructionStr='Use XPS2 Mid (and Input) Mirror to align to xhair!')
    CAMconfig(CAMreq='CompOutNF',RefXhairXY=[364,277],MisalignmentTolerance=40,ImageNo=2, RefXhairXYsize=[40,40],LIVE=False,InstructionStr='Use XPS2 Input(/Mid) Mirror to align to xhair!')
...
def reloadchk():
    print('Reload check: 20220128efg')
